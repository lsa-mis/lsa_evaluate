...
RSpec.describe EntryPolicy do
  ...
  # Award outcome edits share the same container-role gate as disqualification.
  describe '#update_award_outcome?' do
    let(:container) { contest_instance.contest_description.container }
    ...
    context 'for a Collection Administrator in the same container' do
      ...
      it { is_expected.to permit_action(:update_award_outcome) }
    end
    ...
    context 'for a Collection Administrator in a different container' do
      ...
      it { is_expected.to forbid_action(:update_award_outcome) }
    end
